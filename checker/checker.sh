        ...
        then
            ...
            then
                ...
                then
                    ...
                    speedup=$(echo "${seq_time}/${par_time}" | sed 's/,/\./g' | bc -l | xargs printf "%.2f")
                ...
                max=$(echo "${speedup} >= ${full_speedups[$count]}" | sed 's/,/\./g' | bc -l)
                part=$(echo "${speedup} >= ${int_speedups[$count]}" | sed 's/,/\./g' | bc -l)
                if [ $max == 1 ]
                then
